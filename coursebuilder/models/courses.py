...
class Unit13(object):
    ...
    def __init__(self):
        ...
        # if a unit is listed below another unit
    ...
    def _from_dict(self, adict):
        ...
        if unit_dicts:
            for unit_dict in unit_dicts:
                ...
                if unit.type == 'PA':
                    unit.type = verify.UNIT_TYPE_CUSTOM
                    unit.custom_unit_type = (
                        'com.google.coursebuilder.programming_assignment')
                    if 'pa_id' in unit_dict:
                        unit.properties['PA'] = unit_dict['pa_id']
...
class CachedCourse13(AbstractCachedObject):
    ...
    def __init__(
        ...
        self.unit_id_to_child_unit_ids = unit_id_to_child_unit_ids
...
class CourseModel13(object):
    ...
    @classmethod
    def _make_unit_id_to_child_unit_ids_dict(cls, units):
        """Creates an index of unit.unit_id to unit.lessons."""
        unit_id_to_child_unit_ids = {}
        for unit in units:
            if not unit.parent_unit:
                continue
            key = str(unit.parent_unit)
            if key not in unit_id_to_child_unit_ids:
                unit_id_to_child_unit_ids[key] = []
            unit_id_to_child_unit_ids[key].append(str(unit.unit_id))
        return unit_id_to_child_unit_ids
    ...
    @property
    def unit_id_to_child_unit_ids(self):
        return self._unit_id_to_child_unit_ids
    ...
    def _index(self):
        ...
        self._unit_id_to_lesson_ids = self._make_unit_id_to_lessons_lookup_dict(
            self._lessons)
        self._unit_id_to_child_unit_ids = self._make_unit_id_to_child_unit_ids_dict(
            self._units)
    ...
    def save_settings(self, course_settings):
        return course_list.CourseListConnector.save_settings(self, course_settings)
    ...
    def get_subunits(self, unit_id):
        if self._unit_id_to_child_unit_ids == None:
            return []
        unit_ids = self._unit_id_to_child_unit_ids.get(str(unit_id))
        units = []
        if unit_ids:
            for uid in unit_ids:
                units.append(self.find_unit_by_id(uid))
        return units
    ...
    def add_unit(self, unit_type, title, punit, custom_unit_type=None):
    ...
    def import_from(self, src_course, errors):
        ...
        def copy_unit(unit, punit, src_course, errors):
            new_unit = self.add_unit(
                unit.type, unit.title, punit, custom_unit_type=unit.custom_unit_type)
            ...
            if unit.is_custom_unit():
                ...
                if not cu:
                    return None
                cu.import_unit(src_course, unit, self, new_unit)
            return new_unit
        ...
        with common_utils.Namespace(self.app_context.get_namespace_name()):
            for unit in src_course.get_units():
                ...
                if unit.parent_unit:
                    continue
                new_unit = copy_unit(unit, None, src_course, errors)

                for subunit in src_course.get_subunits(unit.unit_id):
                     new_subunit = copy_unit(subunit, new_unit, src_course, errors)
...
class Workflow(object):
    ...
    def _convert_date_string_to_datetime(self, date_str):
        ...
        try:
            ...
            logging.error('date time in not correct format : ' + date_str)
            return None
    ...
    def submit_only_once(self):
        return self.to_dict().get(SUBMIT_ONLY_ONCE)
...
class Course(object):
    ...
    def get_unit_lessons_dict(self):
        ...
        for unit in self.get_units():
            ...
            for lesson in self.get_lessons(unit.unit_id):
                lessons.append(lesson)
            lessons_dict[unit.unit_id] = lessons
        return lessons_dict
    ...
    def get_unit_sub_unit_dict(self):
        ...
        for unit in self.get_units():
            subunits = []
            for subunit in self.get_subunits(unit.unit_id):
                subunits.append(subunit)
            sub_unit_dict[unit.unit_id] = subunits
    ...
    def add_custom_unit(self, unit_type, punit):
    ...
    def is_course_complete(self, student):
        ...
        score_list = self.get_all_scores(student, list_not_available_assesments=True)
        for unit in score_list:
    ...
    def get_all_scores(self, student, list_not_available_assesments=False):
        ...
        for unit in unit_list:
            if not unit.now_available and not list_not_available_assesments:
                continue
            if not unit.scored():
                continue
            ...
            if completed and submission_due_date:
...
""" % (title, admin_email)
    ...
    def should_automatically_subscribe_to_forum(self):
        """Get Announcement email address for the course."""
        return self.get_value_from_course_dict('auto_subscribe_to_forum')

    def get_course_forum_email(self):
        """Get Forum email address for the course."""
        return self.get_value_from_course_dict('forum_email')

    def get_value_from_course_dict(self, key):
        """Returns the value for the key from the course dictionary."""
