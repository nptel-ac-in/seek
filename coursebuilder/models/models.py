...
class PersonalProfile(BaseEntity):
    ...
    profession = db.StringProperty(indexed=False)
    employer_name = db.StringProperty(indexed=False)
...
class PersonalProfileDTO(object):
    ...
    def __init__(self, personal_profile=None):
        ...
        if personal_profile:
            ...
            self.name_of_college = personal_profile.name_of_college
            self.age_group = personal_profile.age_group
            self.graduation_year = personal_profile.graduation_year
            self.employer_name = personal_profile.employer_name
            self.profession = personal_profile.profession
            self.accepted_honor = personal_profile.accepted_honor
            self.accepted_terms = personal_profile.accepted_terms
            self.mobile_number = personal_profile.mobile_number
            self.local_chapter = personal_profile.local_chapter
...
class StudentProfileDAO(object):
    ...
    @classmethod
    def _memcache_email_key(cls, key):
        """Makes a memcache key from primary key."""
        return 'entity:personal-profile:email:%s' % key
    ...
    @classmethod
    def _get_profile_by_email(cls, user_email):
        """Loads profile given a user_id and returns Entity object."""
        old_namespace = namespace_manager.get_namespace()
        try:
            namespace_manager.set_namespace(cls.TARGET_NAMESPACE)
            ...
            profile = MemcacheManager.get(
                cls._memcache_email_key(user_email), namespace=cls.TARGET_NAMESPACE)
            if profile == NO_OBJECT:
                return None
            if profile:
                return profile
            profile = None
            profiles = PersonalProfile.all().filter('email =', user_email).fetch(limit=2)
            if len(profiles) == 1:
                profile = profiles[0]
            MemcacheManager.set(
                cls._memcache_email_key(user_email), profile if profile else NO_OBJECT,
                namespace=cls.TARGET_NAMESPACE)
            return profile
        finally:
            namespace_manager.set_namespace(old_namespace)
    ...
    def _update_global_profile_attributes(
        ...
        course_info=None, country_of_residence=None, state_of_residence=None,
        city_of_residence=None, name_of_college=None, age_group=None,
        ...
        if state_of_residence is not None:
            profile.state_of_residence = state_of_residence

        if city_of_residence is not None:
            profile.city_of_residence = city_of_residence

        if name_of_college is not None:
            profile.name_of_college = name_of_college
        ...
        if employer_name is not None:
            profile.employer_name = employer_name

        if accepted_terms is not None:
            profile.accepted_terms = accepted_terms
        if accepted_honor is not None:
            profile.accepted_honor = accepted_honor

        if mobile_number is not None:
            profile.mobile_number = mobile_number

        if isinstance(local_chapter, bool):
            profile.local_chapter = local_chapter
    ...
    def _update_attributes(
        ...
        email=None, legal_name=None, nick_name=None,
        date_of_birth=None, is_enrolled=None, final_grade=None,
        course_info=None, labels=None,country_of_residence=None, state_of_residence=None,
        ...
        if profile:
            cls._update_global_profile_attributes(
                ...
                employer_name=employer_name, accepted_honor=accepted_honor,
    ...
    @classmethod
    def get_profile_by_email(cls, user_email):
        """Loads profile given a user_id and returns DTO object."""
        profile = cls._get_profile_by_email(user_email)
    ...
    def bulk_get_student_profile_by_id(
        ...
        """Returns a list of profiles of the IDS provided in the list"""
        with common_utils.Namespace(cls.TARGET_NAMESPACE):
            key_list = [
                db.Key.from_path('PersonalProfile', user_id).__str__()
                for user_id in id_list]
            profile_list = db.Model.get(
    ...
    @classmethod
    def bulk_get_student_and_profile_by_id(
            cls, id_list, consistency=datastore.EVENTUAL_CONSISTENCY):
    ...
    def _update_in_txn(
        ...
        cls._update_attributes(
            ...
            state_of_residence=state_of_residence,
            city_of_residence=city_of_residence,
            name_of_college=name_of_college, age_group=age_group, graduation_year=graduation_year,
            profession=profession, employer_name=employer_name,
...
class StudentPropertyEntity(BaseEntity):
    ...
    @classmethod
    def get_for_id(cls, user_id, property_name):
        """Loads student property."""
        key = cls.create_key(user_id, property_name)
        value = MemcacheManager.get(cls._memcache_key(key))
        if NO_OBJECT == value:
            return None
        if not value:
