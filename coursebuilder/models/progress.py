...
class UnitLessonCompletionTracker(object):
    ...
    def get_all_progress(self, user_id):
        """Retrusn a dict with status of each unit/lesson"""
        progress = StudentPropertyEntity.get_for_id(user_id, UnitLessonCompletionTracker.PROPERTY_KEY)
        ...
        for unit in units:
            if unit.type == verify.UNIT_TYPE_ASSESSMENT:
                result[unit.unit_id] = 2 if self.is_assessment_completed(
                    progress, unit.unit_id) else 0
            elif unit.type == verify.UNIT_TYPE_UNIT:
                value = self.get_unit_status(progress, unit.unit_id)
                result[unit.unit_id] = value or 0
            elif unit.type == verify.UNIT_TYPE_CUSTOM:
