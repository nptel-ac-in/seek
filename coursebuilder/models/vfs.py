...
class FileMetadataEntity(BaseEntity):
    ...
    stored_on_gcs = db.BooleanProperty(indexed=False, default=False)
...
class DatastoreBackedFileSystem(object):
    ...
    def gcs_filename(self, filename):
        return '/nptelmooc/%s%s' % (
            namespace_manager.get_namespace(), filename)
    ...
    def open(self, afilename):
        ...
        if not found:
            ...
            if metadata:
                if metadata.stored_on_gcs:
                    gcs_file = gcs.open(self.gcs_filename(filename))
                    if gcs_file:
                        result = FileStreamWrapped(metadata, gcs_file.read())
                        return result
                else:
