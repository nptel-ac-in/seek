...
DISTRIBUTED_LIBS="\
  ...
  yui_2in3-2.9.0.zip \
  yui_3.6.0.zip \
  python-http-client-2.1.1.zip \
  sendgrid-python-3.1.10.zip \
"
...
if need_install google-code-prettify ; then
  echo Installing Google Code Prettify
  curl --location --silent https://storage.googleapis.com/google-code-archive-downloads/v2/code.google.com/google-code-prettify/prettify-small-4-Mar-2013.tar.bz2 -o google_code_prettify.tar.bz2
  tar -jxf google_code_prettify.tar.bz2
  zip -r google-code-prettify.zip google-code-prettify
  rm google_code_prettify.tar.bz2
  rm -rf google-code-prettify
  mv google-code-prettify.zip  $DISTRIBUTED_LIBS_DIR/
fi
