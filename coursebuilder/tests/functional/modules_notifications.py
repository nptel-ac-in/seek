...
class ModelTestBase(actions.TestBase):
    ...
    def assert_for_export_removes_denylisted_fields(self, unsafe_model):
        safe_model = unsafe_model.for_export(self.transform_fn)

        for denylisted_prop in self.ENTITY_CLASS._PROPERTY_EXPORT_DENYLIST:
            self.assertTrue(hasattr(unsafe_model, denylisted_prop.name))
            self.assertFalse(hasattr(safe_model, denylisted_prop.name))
...
class NotificationTest(ModelTestSpec, ModelTestBase):
    ...
    def test_for_export_transforms_to_and_sender_and_strips_denylist(self):
        ...
        self.assert_for_export_removes_denylisted_fields(unsafe)
...
class PayloadTest(ModelTestSpec, ModelTestBase):
    ...
    def test_for_export_denylists_data(self):
        self.assert_for_export_removes_denylisted_fields(self.payload)
