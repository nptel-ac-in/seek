...
class TestUserRoutes(actions.TestBase):
    ...
    def test_override_slash(self):
        ...
        # Since courses are not browsable by default
        # It will get forwarded to preview
        response = self.get('')
        self.assertEquals(self.times_routed, 0)
        self.assertEquals(response.status_code, 302)
        self.assertIn('drive-course/preview', response.headers["Location"])
        response = self.get('/drive-course/preview')
        self.assertEquals(self.times_routed, 0)
