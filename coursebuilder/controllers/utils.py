...
def is_date_time_specified_correctly(value, errors):
    if not value:
        return
    try:
        datetime.datetime.strptime(value, ISO_8601_DATE_FORMAT)
...
MIN_PROFILE_UPDATE = ConfigProperty(
    'min_profile_timestamp', str, (
        'A student profile must be udpated after this time'),
    '', validator=is_date_time_specified_correctly)

# Date format string for displaying datetimes in UTC.
# Example: 2013-03-21 13:00 UTC
HUMAN_READABLE_DATETIME_FORMAT = '%Y-%m-%d, %H:%M %Z'
...
class CBRequestHandler(webapp2.RequestHandler):
    """A handler that uses local caches."""
    def user_agent(self):
        return httpagentparser.detect(
            self.request.headers.get('User-Agent'), fill_none=True)
    ...
    def get_profile_for_user(self, user):
        return StudentProfileDAO.get_profile_by_user_id(user.user_id())

    @classmethod
    def needs_profile_update(cls, profile):
        if not profile:
            return False

        if profile.local_chapter is None:
            return True

        if not MIN_PROFILE_UPDATE.value:
            return False
        try:
            min_time = datetime.datetime.strptime(
                MIN_PROFILE_UPDATE.value, ISO_8601_DATE_FORMAT)
            return profile.last_updated < min_time
        except Exception as e:  # pylint: disable=broad-except
            logging.info(e)
        return False

    @classmethod
    def needs_to_view_announcements(cls, student, app_context):
        if student is None:
            return False

        if not app_context.last_announcement:
            return False
        ...
        if not additional_fields:
            return False
        if additional_fields.has_key('annoucements_last_viewed'):
            try:
                annoucements_last_viewed = datetime.datetime.strptime(
                    additional_fields['annoucements_last_viewed'],
                    courses.ISO_8601_DATE_FORMAT)
                if app_context.last_announcement <= annoucements_last_viewed:
                    return False
            except Exception as e:  # pylint: disable=broad-except
                logging.error("Excpetion while parsing last_announcement in needs_to_view_announcements")
                logging.error(e)
                #its better not to force notifications view if there is an error.
                return False
        return True
...
class CourseHandler(ApplicationHandler):
    ...
    def get_unit_sub_unit_dict(self):
        return self.get_course().get_unit_sub_unit_dict()
    ...
    def render(self, template_file, additional_dirs=None, save_location=True):
        ...
        if user:
            ...
            if student and student.is_enrolled:
                self.template_value['is_enrolled'] = True
                mentor = Mentor.get_by_id(student.user_id)
                is_mentor = True if mentor is not None else False
                self.template_value['is_mentor'] = is_mentor
            else:
                self.template_value['is_enrolled'] = False
            evaluator = course_staff.CourseStaff.get(user.user_id())
            if evaluator:
                self.template_value['is_course_staff'] = True
        else:
            self.template_value['is_enrolled'] = False

        course = self.get_course()
        if course:
            self.template_value['mentor_enabled'] = (
                mentor_base.MentorBase.can_use_mentor_feature(course))
...
class BaseProfileHandler(object):
    ...
    def fill_profile_data(self, profile):
        ...
        self.template_value['current_mobile_number'] = '+91'
        if profile.mobile_number:
            self.template_value['current_mobile_number'] = profile.mobile_number

        if profile.country_of_residence:
            self.template_value['country_of_residence'] = (
                profile.country_of_residence)

        if profile.state_of_residence:
            self.template_value['state_of_residence'] = (
                profile.state_of_residence)

        if profile.city_of_residence:
            self.template_value['city_of_residence'] = (
                profile.city_of_residence)

        if profile.name_of_college:
            self.template_value['name_of_college'] = (
                profile.name_of_college)

        if profile.age_group:
            self.template_value['age_group'] = (
                profile.age_group)

        if profile.graduation_year:
            self.template_value['graduation_year'] = (
                profile.graduation_year)

        if profile.profession:
            self.template_value['profession'] = (
                profile.profession)
    ...
    def update_profile_data(self, user, profile_only=False):
        ...
        if "IN" == country_of_residence:
            state_of_residence = self.request.get('state_of_residence')
        else:
            state_of_residence = self.request.get('non_indian_state')
        ...
        if college_id and profession in ['student','faculty'] :
            ...
            local_chapter_obj = (
                local_chapter_model
                .LocalChapterDAO.get_local_chapter_by_key(college_id))
            if not local_chapter_obj:
        ...
        StudentProfileDAO.update(user_id, email,
                                 ...
                                 name_of_college=college,
                                 age_group=age_group,
                                 graduation_year=graduation_year,
                                 profession=profession,
                                 employer_name=employer_name,
                                 accepted_terms=accepted_terms,
                                 accepted_honor=accepted_honor,
...
class RegisterHandler(BaseHandler, BaseProfileHandler):
    ...
    def get(self):
        ...
        self.template_value['submit_button_name'] = 'Join'
    ...
    def post(self):
        ...
        registration_options = self.app_context.get_environ()['reg_form']
        can_register = self.app_context.get_environ(
            )['reg_form']['can_register']
        if not can_register:
            self.redirect('/course#registration_closed')
        ...
        if 'name_from_profile' not in self.request.POST.keys():
            # Update the profile
    ...
    def get_admin_service(cls):
        return google_service_account.GoogleServiceManager.get_service(
            name='admin', version='directory_v1')

    def EnrollToGroup(self, user, email):
        service = self.get_admin_service()
        if not service:
            return False
        ...
        try:
            members.insert(groupKey=email, body=data).execute()
            return True
        except Exception as e:
            return False

    def EnrollStudentToGroups(self, user):
        course = self.get_course()
        email = course.get_course_announcement_list_email()
        ret = True
        if email:
            ret = self.EnrollToGroup(user, email)
        if not course.should_automatically_subscribe_to_forum():
            return ret
        email = course.get_course_forum_email()
        if email:
            ret2 = self.EnrollToGroup(user, email)
            ret = ret and ret2
        return ret
