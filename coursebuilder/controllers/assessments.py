...
class AnswerHandler(BaseHandler):
    ...
    def post(self):
        ...
        if submit_only_once:
            if utils.has_score(student, assessment_type):
                self.render('resubmission_not_allowed.html')
                return
        ...
        if grader == courses.HUMAN_GRADER:
            ...
            if not previously_submitted:
                ...
                if time_now > submission_due_date:
                    self.template_value['time_now'] = time_now.replace(tzinfo=timezones.UTC).astimezone(timezones.IST).strftime(
