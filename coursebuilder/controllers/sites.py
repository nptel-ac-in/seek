...
DEFAULT_CACHE_CONTROL_MAX_AGE = 604800
...
def set_static_resource_cache_control(handler):
    ...
    handler.response.expires = None
    handler.response.pragma = None
...
class ApplicationContext(object):
    ...
    def __init__(self, site_type, slug, homefolder, namespace, fs, raw=None,
        ...
        self._fs = fs
        self._raw = raw
        self._course_list_item = course_list_item
    ...
    def title(self):
        if self._course_list_item:
            return self._course_list_item.title
        course = self.get_environ().get('course')
        return course.get('title') if course else 'UNTITLED COURSE'

    @property
    def browsable(self):
        if self._course_list_item:
            return self._course_list_item.browsable
        course = self.get_environ().get('course')
        return course.get('browsable', False) if course else False
    ...
    def can_register(self):
        if self._course_list_item:
            return self._course_list_item.can_register
        reg_form = self.get_environ().get('reg_form')
        return reg_form and reg_form.get('can_register')

    @property
    def registration(self):
        if self._course_list_item:
            return self._course_list_item.registration
        reg_form = self.get_environ().get('reg_form')
        if reg_form.get('opening_soon', False):
            return 'OPENING_SHORTLY'
    ...
    def blurb(self):
        if self._course_list_item:
            return self._course_list_item.blurb
        explorer = self.get_environ().get('explorer')
        return explorer.get('blurb') if explorer else ''

    @property
    def category(self):
        if self._course_list_item:
            return self._course_list_item.category
        explorer = self.get_environ().get('explorer')
        return explorer.get('category') if explorer else ''
    ...
    def last_announcement(self):
        if self._course_list_item:
            return self._course_list_item.last_announcement
        explorer = self.get_environ().get('explorer')
        return explorer.get('last_announcement') if explorer else None
    ...
    def get_title(self):
        if self._course_list_item:
            return self._course_list_item.title if self._course_list_item.title else 'UNTITLED'
        try:
...
def _build_app_context_from_course_list_item(item, create_vfs=True):
    if item.filesystem_path:
        folder = item.filesystem_path
        # pylint: disable-msg=g-long-lambda
        create_fs = lambda unused_ns: LocalReadOnlyFileSystem(
            logical_home_folder=item.folder)
    else:
        folder = '/'
        # pylint: disable-msg=g-long-lambda
    ...
    return ApplicationContext('course', item.slug, folder, namespace, vfs,
...
def _build_course_list_from_db(create_vfs=True):
    ...
    for item in courses:
        all_contexts.append(_build_app_context_from_course_list_item(item))
    _validate_appcontext_list(all_contexts)
    return all_contexts

def get_course_list_from_config(rules_text):
    return _build_course_list_from(rules_text)
...
def get_app_context_for_namespace(namespace):
    ...
    if USE_COURSE_LIST.value:
        clo = course_list.CourseListDAO.get_course_for_namespace(namespace)
        if not clo:
            debug('No app_context in namespace: %s' % namespace)
            return None
        return _build_app_context_from_course_list_item(clo)
...
def get_course_for_path(path):
    ...
    if len(path_parts) > 1:
        ...
        slug = '/'
    clo = course_list.CourseListDAO.get_course_for_path(slug)
    if not clo:
        clo = course_list.CourseListDAO.get_course_for_path('/')
    if not clo:
        return None
    return _build_app_context_from_course_list_item(clo)
