        ...
        try:
            ...
            if user is None:
                ...
                if self.needs_profile_update(profile):
                    self.redirect('/explorer/profile?action=edit&force=true', normalize=False)
                    return
...
class AssessmentHandler(BaseHandler):
    ...
    def get_assessment_content(self, student, course, unit, as_lesson):
        ...
        if submit_only_once:
            if utils.has_score(student, str(unit.unit_id)):
                readonly_view = True
        submission_due_date = unit.workflow.get_submission_due_date()
        if submission_due_date:
            self.template_value['submission_due_date'] = (
                submission_due_date.replace(tzinfo=timezones.UTC).astimezone(timezones.IST).strftime(HUMAN_READABLE_DATETIME_FORMAT))
    ...
    def configure_active_view_1_5(self, unit, submission_contents):
        ...
        self.template_value['enable_negative_marking'] = unit.enable_negative_marking
        if submission_contents:
            # If a previous submission exists, reinstate it.
            self.template_value['html_saved_answers'] = transforms.dumps(
                submission_contents)
