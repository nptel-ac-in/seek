...
class UnitLessonEditor(ApplicationHandler):
    ...
    def post_add_link(self):
        ...
        course = courses.Course(self)
        first_unit = self.get_first_unit(course)
        if first_unit:
            link = course.add_link(first_unit)
            link.href = ''
            course.save()
            self.redirect(self.get_action_url(
    ...
    def get_first_unit(self, course):
        for unit in course.get_units():
            if unit.type == verify.UNIT_TYPE_UNIT:
                return unit
        return None
    ...
    def post_add_assessment(self):
        ...
        first_unit = self.get_first_unit(course)
        if first_unit:
            assessment = course.add_assessment(first_unit)
            course.save()
            self.redirect(self.get_action_url(
    ...
    def post_add_custom_unit(self):
        ...
        first_unit = self.get_first_unit(course)
        custom_unit = course.add_custom_unit(custom_unit_type, first_unit)
        course.save()
        self.redirect(self.get_action_url(
    ...
    def get_edit_link(self):
        ...
        self._render_edit_form_for(
            ...
            annotations_dict=LinkRESTHandler.get_schema_annotations_dict(
                courses.Course(self), int(self.request.get('key'))))
    ...
    def get_edit_assessment(self):
        ...
        self._render_edit_form_for(
            ...
            annotations_dict=AssessmentRESTHandler.get_schema_annotations_dict_assessment(
                courses.Course(self), int(self.request.get('key'))))
