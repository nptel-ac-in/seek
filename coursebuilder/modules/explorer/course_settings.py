...
def register():
    courses.Course.OPTIONS_SCHEMA_PROVIDERS[
        courses.Course.SCHEMA_SECTION_COURSE] += [
            ...
            lambda _: schema_fields.SchemaField(
                'course:' + constants.FEATURED, 'Featured',
                'boolean',
                description=messages.COURSE_FEATURED_DESCRIPTION,
                optional=True, i18n=False, default_value=False,
            ),
            lambda _: schema_fields.SchemaField(
                'course:' + constants.EXPLORER_SUMMARY, 'Explorer Summary',
                'html', description=messages.EXPLORER_SUMMARY_DESCRIPTION,
                optional=True, i18n=False
            ),
            lambda _: schema_fields.SchemaField(
                'course:' + constants.EXPLORER_INSTRUCTOR_NAME,
                'Explorer Instructor Name', 'string',
                description=messages.EXPLORER_INSTRUCTOR_NAME_DESCRIPTION,
                optional=True, i18n=False
            ),
            lambda _: schema_fields.SchemaField(
                'course:' + constants.TAGS,
                'Explorer Tags', 'array',
