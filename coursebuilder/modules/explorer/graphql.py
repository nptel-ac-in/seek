...
from modules.gql.gql import CourseCategory
...
class Site(graphene.ObjectType):
    ...
    def resolve_course_explorer(self, args, info):
        return CourseExplorer(
            extra_content=self.data.get('extra_content'),
            youtube_video=self.data.get('youtube_video'))
...
# Resolver functions for Course type object
def resolve_estimated_workload_for_course(gql_course, args, info):
    return courses.Course.get_named_course_setting_from_environ(
        'estimated_workload', gql_course.course_environ)

def resolve_category_for_course(gql_course, args, info):
...
def register():
    ...
    gql.Course.add_to_class(
        'estimated_workload', graphene.String(
            resolver=resolve_estimated_workload_for_course))
    gql.Course.add_to_class(
        'category', graphene.Field(CourseCategory,
                                   resolver=resolve_category_for_course))
