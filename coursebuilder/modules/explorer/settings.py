...
GCB_ENABLE_COURSE_EXPLORER_PAGE = config.ConfigProperty(
    ...
    messages.SITE_SETTINGS_COURSE_EXPLORER, default_value=True,
...
# COURSE_NAVBAR_LINKS = config.ConfigProperty(
#     'gcb_explorer_navbar_links', list, [],
#     label='Explorer page ', show_in_site_set
# )
...
def schema_provider(unused_course):
    ...
    navbar_link = schema_fields.FieldRegistry('Navbar Link', '')
    navbar_link.add_property(
        schema_fields.SchemaField(
            'title', 'Title', 'string', optional=False))
    navbar_link.add_property(
        schema_fields.SchemaField(
            'href', 'URL', 'string', optional=False))
    navbar_link.add_property(
        schema_fields.SchemaField(
            'target', 'Open in', 'string',
            select_data=(('', 'Same tab'), ('_blank', 'New tab')),
            optional=True))

    group.add_property(schema_fields.FieldArray(
        'navbar_links_left', 'Navbar Static Links (left)',
        item_type=navbar_link,
        description='Static links to be displayed on the left side of the navbar.',
        optional=True,
    ))

    group.add_property(schema_fields.FieldArray(
        'navbar_links_right', 'Navbar Static Links (right)',
        item_type=navbar_link,
        description='Static links to be displayed on the left right of the navbar.',
        optional=True,
    ))
    ...
    group.add_property(schema_fields.SchemaField(
        'youtube_video', 'Youtube Embed URL', 'string',
        description=messages.YOUTUBE_VIDEO_DESCRIPTION,
        i18n=False,
        optional=True,
    ))
...
class GlobalSettingsEditorCustomAction(
    ...
    @classmethod
    def get(cls, handler):
        if not super(GlobalSettingsEditorCustomAction, cls).get(handler):
            return
        schema = schema_provider(None)
        cls.render_content(handler, oeditor.ObjectEditor.get_html_for(
            handler, schema.get_json_schema(), schema.get_schema_dict(), 'key',
...
def register():
    GlobalSettingsEditorCustomAction.add_to_menu(
...
global_routes = [
    (SettingsEditorRest.URL, SettingsEditorRest),
]
