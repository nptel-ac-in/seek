...
class MilestoneTrigger(AvailabilityTrigger):
    ...
    @classmethod
    def get_course_when_from_string(cls, when, title, course_name):
        # Non-empty 'when' will always be an ISO 8601 "UTC Z" string.
        # Also adding some backward compatibility for older courses.
        try:
            ...
            logging.error("Failed to parse course date course: %s error: %s",
                          course_name, str(e))
    ...
    def get_course_when(cls, env, milestone, course_name):
        ...
        return cls.get_course_when_from_string(when, title, course_name)

    @classmethod
    def get_course_when_from_date_str(cls, date_str, milestone, course_name):
        if not date_str:
            return cls.CourseWhen('', '', '', '', '')

        title = constants.MILESTONE_TO_TITLE.get(milestone)
        # title = policy.get('title') or title
        return cls.get_course_when_from_string(date_str, title, course_name)
