...
class UnitLessonEditor(object):
    ...
    @classmethod
    def get_target_unit(cls, course, handler):
        if handler.request.get('unit_id'):
            return course.find_unit_by_id(handler.request.get('unit_id'))
        else:
    ...
    def post_add_lesson(cls, handler):
        ...
        target_unit = cls.get_target_unit(course, handler)
        if target_unit:
    ...
    def post_add_link(cls, handler):
        ...
        target_unit = cls.get_target_unit(course, handler)
        if target_unit:
            link = course.add_link(target_unit)
            link.href = ''
            course.save()
            handler.redirect(handler.get_action_url(
    ...
    def post_add_assessment(cls, handler):
        ...
        target_unit = cls.get_target_unit(course, handler)
        if target_unit:
            assessment = course.add_assessment(target_unit)
            course.save()
            handler.redirect(handler.get_action_url(
    ...
    def post_add_custom_unit(cls, handler):
        ...
        if target_unit:
            ...
            custom_unit = course.add_custom_unit(custom_unit_type, target_unit)
            course.save()
            handler.redirect(handler.get_action_url(
...
class CommonUnitRESTHandler(utils.BaseRESTHandler):
    ...
    def delete(self):
        ...
        if unit.is_locked and unit.is_assessment():
            transforms.send_json_response(
                self, 412, "Couldn't delete. This unit is locked.", {'key': key})
            return

        if course.delete_unit(unit):
            course.save()
            transforms.send_json_response(self, 200, 'Deleted.')
        else:
            transforms.send_json_response(
                self, 412, "Couldn't delete. This unit has child units or lessons.", {'key': key})
            return



    @classmethod
    def get_schema_annotations_dict(cls, course, this_unit_id):
        schema = resources_display.ResourceLink.get_schema(course, this_unit_id)
        unit_list = []
        for unit in course.get_units():
            if unit.type == 'U':
                unit_list.append(
        ...
        schema.get_property('parent_unit').set_select_data(unit_list)
        return schema.get_schema_dict()

    @classmethod
    def get_schema_annotations_dict_assessment(cls, course, this_unit_id):
        schema = resources_display.ResourceAssessment.get_schema(course, this_unit_id)
        unit_list = []
        for unit in course.get_units():
            if unit.type == 'U':
                unit_list.append(
        ...
        extra_select_options = dict()
        extra_select_options['parent_unit'] = unit_list
        schema.get_sub_registry("assessment").get_property('parent_unit').set_select_data(unit_list)
        return schema.get_schema_dict()
...
class ImportCourseRESTHandler(utils.BaseRESTHandler):
    ...
    def _get_course_list(cls):
        ...
        for acourse in sites.get_all_courses():
            ...
            course_list.append({
                'value': acourse.get_slug(), 'label': cgi.escape(atitle)})
        return course_list
    ...
    def put(self):
        ...
        source = None
        for acourse in sites.get_all_courses():
            if acourse.get_slug() == course_raw:
                source = acourse
                break
