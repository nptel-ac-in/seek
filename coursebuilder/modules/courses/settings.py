...
class CourseSettingsDisplayHelper(object):
    ...
    def build_settings_property(cls, setting_dict, environ):
        ...
        if setting_dict.get('repeated', False):
            content = safe_dom.NodeList()
            if prop_value:
                for v in prop_value:
                    content.append(cls.build_settings_section(
                        setting_dict.get('item_type'), v))
            else:
                content.append(cls.build_settings_section(
                    setting_dict.get('item_type'), {}))
            return content
...
class CourseYamlRESTHandler(controllers_utils.BaseRESTHandler):
    ...
    def get_course_dict(self):
        ...
        # TODO(rthakker) remove this when not requred anymore
        data = self.get_course().get_environ(self.app_context)
        course_dict = data.get('course', dict())
        main_image = course_dict.get('main_image', dict())
        if not main_image.get('url'):
            main_video = course_dict.get('main_video', dict())
            if main_video:
                main_image['url'] = main_video.get('url')
        return data
...
class CourseSettingsRESTHandler(CourseYamlRESTHandler):
	...
	'inputex-hidden', 'inputex-list', 'inputex-integer',
    ]
    ...
    def get_form_base_url(self, email):
        if not email:
            return None
        if '@googlegroups.com' not in email:
            return 'https://groups.google.com/a/' + email.split('@')[1]
        return 'https://groups.google.com'
    ...
    def get_groups_web_url(self, email):
        ...
        if not group_id:
            return None
        return self.get_form_base_url(email) + '/group/' + group_id

    def get_groups_embed_url(self, email):
        group_id = self.get_group_id(email)
        if not group_id:
            return None
        return self.get_form_base_url(email) + '/forum/embed/?place=forum/' + group_id
    ...
    def process_put(self, request, payload):
        ...
        if 'course' in request_data:
            logging.info('course part or request data')
            self._process_course_data(request_data['course'])
        else:
            logging.info('course not part or request data')
        logging.info(request_data)
