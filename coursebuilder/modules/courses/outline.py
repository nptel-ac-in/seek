...
def _render_lesson_outline(handler, course, unit, lesson):
...
def _render_unit_outline(handler, course, unit, total_rendered):
    course_writable = handler.app_context.is_editable_fs()
    can_view_props = course_writable and permissions.can_edit(
        handler.app_context, constants.SCOPE_UNIT)
    ...
    for lesson in lessons:
        lessons_rendered.append(
            _render_lesson_outline(handler, course, unit, lesson))
        total_rendered['lesson_ids'].append(lesson.lesson_id)

    unit_data['lessons'] = lessons_rendered
    ...
    if sub_units:
        ...
        for sub_unit in sub_units:
            if sub_unit.type == verify.UNIT_TYPE_LINK:
                sub_unit_rendered.append(_render_link_outline(handler, sub_unit))
            elif sub_unit.type == verify.UNIT_TYPE_CUSTOM:
                sub_unit_rendered.append(_render_custom_unit_outline(handler, course, sub_unit))
            elif sub_unit.type == verify.UNIT_TYPE_ASSESSMENT:
                assessment_outline = _render_assessment_outline(handler, sub_unit)
                assessment_outline['class'] = 'assessment'
                sub_unit_rendered.append(assessment_outline)
                total_rendered['sub_unit_ids'].append(sub_unit.unit_id)
...
def _render_course_outline_to_html(handler, course):
    ...
    for unit in course.get_units():
        ...
        if unit.type == verify.UNIT_TYPE_ASSESSMENT:
            ...
            units.append(
                _render_unit_outline(handler, course, unit, total_rendered))
    ...
    # Check for any mismatches leading to invisible lessons
    invisible_lessons = []
    if len(total_rendered['lesson_ids']) < len(course.lessons):
        for lesson in course.lessons:
            if lesson.lesson_id not in total_rendered['lesson_ids']:
                invisible_lessons.append(_render_lesson_outline(
                    handler, course, course.get_unit_for_lesson(lesson),
                    lesson))
    ...
    template_values = {
        ...
        'units': units,
        'invisible_lessons': invisible_lessons,
        ...
        'unit_level_actions':unit_level_actions
    }
