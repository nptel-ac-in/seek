                ...
                if unit_order['id'] == unit.unit_id:
                    ...
                    for child in unit_order["children"]:
                        ...
                        if child["section"] == 'sub_unit':
                            sub_unit = course.find_unit_by_id(child["id"])
                            if sub_unit and sub_unit.type == verify.UNIT_TYPE_ASSESSMENT:
                                q_keys, contents = _add_sub_assessment(sub_unit, sub_unit)
                                if q_keys:
                                    question_keys += q_keys
                                    if len(q_keys) > 1:
                                        question_keys += ['subtotal']
                ...
                if unit_order['id'] == unit.unit_id:
                    ...
                    for child in unit_order["children"]:
                        ...
                        if child["section"] == 'sub_unit':
                            sub_unit = course.find_unit_by_id(child["id"])
                            if sub_unit and sub_unit.type == verify.UNIT_TYPE_ASSESSMENT:
                                titles.append('%s: %s' % (unit.title, sub_unit.title))
                                add_index(sub_unit.unit_id, None)
                    break# found matching unit so break
