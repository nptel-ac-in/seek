...
class AssessmentHandler(AssignmentsModuleMixin, utils.BaseHandler):
    ...
    def get_assessment_content(
        ...
        self.template_value['enable_negative_marking'] = transforms.dumps(unit.enable_negative_marking)
        ...
        submit_only_once = unit.workflow.is_single_submission()
        self.template_value['submit_only_once'] = submit_only_once
        # TODO(rthakker) Use either submit_only_once or is_single_submission
            ...
            self.template_value['submission_due_date'] = (
                submission_due_date.replace(
                    tzinfo=timezones.UTC).astimezone(timezones.IST).strftime(
                        utils.HUMAN_READABLE_DATETIME_FORMAT.replace('UTC', 'IST')))
            ...
            if not student.is_transient:
                ...
                if submission is not None:
                    ...
                    if due_date_exceeded and unit.workflow.show_feedback():
                        score = course.get_score(student, str(unit.unit_id))
                        self.template_value['show_feedback'] = True
                        self.template_value['score'] = '%d' % (score)
    ...
    def get_review_received_1_5(self, unit, review):
        if review:
            return {
                ...
                'saved_answers': review.contents,
                'score': review.score
            }
        return { 'content': unit.html_review_form }
...
class AnswerHandler(AssignmentsModuleMixin, utils.BaseHandler):
    ...
    def post(self):
        ...
        self.template_value['unit_id'] = unit.unit_id
        self.template_value['parent_unit_id'] = unit.parent_unit

        # Check if submit only once
        submit_only_once = unit.workflow.is_single_submission()
        self.template_value['submit_only_once'] = submit_only_once

        submission = student_work.Submission.get(
            unit.unit_id, student.get_key())
        if submission is not None and submit_only_once:
            self.error(403)
            self.template_value['error'] = True
            self.template_value['error_text'] = (
        ...
        if grader == courses.AUTO_GRADER:
            score = scorer.score_assessment(answers, unit.html_content, enable_negative_marking)
