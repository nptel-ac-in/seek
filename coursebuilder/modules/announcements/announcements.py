...
ANNOUNCEMENTS_INTENT = "courseannouncements"
...
class AnnouncementsStudentHandler(
        AnnouncementsHandlerMixin, BaseHandler,
    ...
    def get_list(self):
        ...
        if student and len(items) > 0:
            ...
            additional_fields['annoucements_last_viewed'] = datetime.datetime.now().strftime(
...
class AnnouncementsItemRESTHandler(BaseRESTHandler):
    ...
    @classmethod
    def SCHEMA(cls, announcement_email):
...
class AnnouncementEntity(entities.BaseEntity):
    ...
    def _translate_content(cls, items):
        ...
        fake_items = [
            ...
            for item in items]
        i18n_dashboard.translate_dto_list(course,app_context, fake_items, key_list)
...
class AnnouncementUtil():
    @classmethod
    def update_course_with_last_announcement(cls, namespace):
        latest_announcements =  AnnouncementEntity.all().filter(
            'is_draft =',False).order('-date').fetch(limit=1)
        if latest_announcements and len(latest_announcements) > 0:
            latest_announcement = latest_announcements[0]
            course_list.CourseListDAO.update_course_details(namespace,
                            last_announcement=datetime.datetime(
                                year=latest_announcement.date.year,
                                month=latest_announcement.date.month,
                                day=latest_announcement.date.day)
                            )
        else:
            #the latest anoucement is deleted or unpublished then make the date to 2000/01/01
            course_list.CourseListDAO.update_course_details(namespace,
                                last_announcement=datetime.datetime(
                                    year=2000,
                                    month=1,
