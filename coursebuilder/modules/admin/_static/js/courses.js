...
$(function() {
  ...
  EditMultiCourseCategoryPanel.prototype._getFormFields = function() {
   ...
   $.ajax({
        url : "/rest/modules/courses/category_list", dataType: 'text'
    }).done(function(data_response)  {
       payload_data = window.gcb.parseJsonResponse(data_response);
       payload =  JSON.parse(payload_data.payload)
       category_list = payload.categories;
      $.each(category_list,function(key, value) 
      {
            $('#multi-course-category').append('<option value=' + key + '>' + value + '</option>');
      });
    }).fail(function()  {
  ...
  EditMultiCourseIsFeaturedPanel = function () {
    EditMultiCoursePanel.call(this);
  };
  EditMultiCourseIsFeaturedPanel.prototype = new EditMultiCoursePanel();
  EditMultiCourseIsFeaturedPanel.prototype._getTitle = function() {
    return 'Set Whether Course Is featured';
  };
  EditMultiCourseIsFeaturedPanel.prototype._getXsrfToken = function() {
    return $('#edit_multi_course_is_featured').data('xsrfToken');
  };
  EditMultiCourseIsFeaturedPanel.prototype._getFormFields = function() {
    var ret = $(
        '<div class="form-row">' +
        '  <label>In Featured?' +
        '    <div> ' +
        ...
        '        <input ' +
        '          type="radio" ' +
        '          id="multi-course-is-featured-yes" ' +
        '          name="is_featured"> Yes ' +
        '      </label> ' +
        '    </div> ' +
        '    <div> ' +
        '      <label class="multi-course-show-in-explorer-no"> ' +
        '        <input ' +
        '          type="radio" ' +
        '          id="multi-course-is-featured-no" ' +
        '          name="is_featured"> No ' +
        '      </label> ' +
        '    </div> ' +
        '  </label>' +
    ...
    this._is_featured_yes = ret.find('#multi-course-is-featured-yes');
    this._is_featured_no = ret.find('#multi-course-is-featured-no');
    var that = this;
    this._is_featured_yes.change(function(event) {
      that._is_featured_yes.parent().removeClass('input-invalid');
      that._is_featured_no.parent().removeClass('input-invalid');
    });
    this._is_featured_no.change(function(event) {
      that._is_featured_yes.parent().removeClass('input-invalid');
      that._is_featured_no.parent().removeClass('input-invalid');
    });
    return ret;
  };
  EditMultiCourseIsFeaturedPanel.prototype._validate = function() {
    var is_yes = this._is_featured_yes.prop('checked');
    var is_no = this._is_featured_no.prop('checked');
    if (is_yes == is_no) {
      this._is_featured_yes.parent().addClass('input-invalid');
      this._is_featured_no.parent().addClass('input-invalid');
    } else {
      this._is_featured_yes.parent().removeClass('input-invalid');
      this._is_featured_no.parent().removeClass('input-invalid');
    }
    return is_yes != is_no;
  };
  EditMultiCourseIsFeaturedPanel.prototype._canBeCleared = function() {
    return false;
  };
  EditMultiCourseIsFeaturedPanel.prototype._getCurrentValue = function(
      courseNamespace) {
    return $('#is_featured_' + courseNamespace).text();
  };
  EditMultiCourseIsFeaturedPanel.prototype._getSaveUrl = function() {
    return '/rest/course/settings';
  };
  EditMultiCourseIsFeaturedPanel.prototype._getSavePayload = function() {
    var is_featured = this._is_featured_yes.prop('checked');
    return {
      'homepage': {
        'course:featured': is_featured
      }
    };
  };
  EditMultiCourseIsFeaturedPanel.prototype._settingSaved = (
      function(payload) {
        console.log(payload);
        console.log(this._is_featured_yes);
        var courseNamespace = payload.key;
        var message = '';
        if (this._is_featured_yes.prop('checked')) {
          message = 'Yes';
        } else {
          message = 'No';
        }
        $('#is_featured_' + courseNamespace).text(message);
        $('#current_value_' + courseNamespace).text(message);
      }
  );
  function editMultiCourseIsFeatured() {
    // Var name intentionally in global namespace as hook for tests to modify.
    window.gcb_multi_edit_dialog = new EditMultiCourseIsFeaturedPanel();
    window.gcb_multi_edit_dialog.init();
    window.gcb_multi_edit_dialog.open();
  };

EditMultiCourseMentorSPOCPanel = function () {
    EditMultiCoursePanel.call(this);
  };
  EditMultiCourseMentorSPOCPanel.prototype = new EditMultiCoursePanel();
  EditMultiCourseMentorSPOCPanel.prototype._getTitle = function() {
    return 'Set Whether Course has Mentor SPOC';
  };
  EditMultiCourseMentorSPOCPanel.prototype._getXsrfToken = function() {
    return $('#edit_multi_course_mentor_spoc').data('xsrfToken');
  };
  EditMultiCourseMentorSPOCPanel.prototype._getFormFields = function() {
    var ret = $(
        '<div class="form-row">' +
        '  <label>Mentor SPOC?' +
        '    <div> ' +
    ...
    this._mentor_spoc_yes = ret.find('#multi-course-is-featured-yes');
    this._mentor_spoc_no = ret.find('#multi-course-is-featured-no');
    var that = this;
    this._mentor_spoc_yes.change(function(event) {
      that._mentor_spoc_yes.parent().removeClass('input-invalid');
      that._mentor_spoc_no.parent().removeClass('input-invalid');
    });
    this._mentor_spoc_no.change(function(event) {
      that._mentor_spoc_yes.parent().removeClass('input-invalid');
      that._mentor_spoc_no.parent().removeClass('input-invalid');
    });
    return ret;
  };
  EditMultiCourseMentorSPOCPanel.prototype._validate = function() {
    var is_yes = this._mentor_spoc_yes.prop('checked');
    var is_no = this._mentor_spoc_no.prop('checked');
    if (is_yes == is_no) {
      this._mentor_spoc_yes.parent().addClass('input-invalid');
      this._mentor_spoc_no.parent().addClass('input-invalid');
    } else {
      this._mentor_spoc_yes.parent().removeClass('input-invalid');
      this._mentor_spoc_no.parent().removeClass('input-invalid');
    }
    return is_yes != is_no;
  };
  EditMultiCourseMentorSPOCPanel.prototype._canBeCleared = function() {
    return false;
  };
  EditMultiCourseMentorSPOCPanel.prototype._getCurrentValue = function(
      courseNamespace) {
    return $('#mentor_spoc_' + courseNamespace).text();
  };
  EditMultiCourseMentorSPOCPanel.prototype._getSaveUrl = function() {
    return '/rest/course/settings';
  };
  EditMultiCourseMentorSPOCPanel.prototype._getSavePayload = function() {
    var mentor_spoc = this._mentor_spoc_yes.prop('checked');
    return {
      'homepage': {
        'course:spoc_mentor': mentor_spoc
      }
    };
  };
  EditMultiCourseMentorSPOCPanel.prototype._settingSaved = (
      function(payload) {
        console.log(payload);
        console.log(this._mentor_spoc_yes);
        var courseNamespace = payload.key;
        var message = '';
        if (this._mentor_spoc_yes.prop('checked')) {
          message = 'Yes';
        } else {
          message = 'No';
        }
        $('#mentor_spoc_' + courseNamespace).text(message);
        $('#current_value_' + courseNamespace).text(message);
      }
  );


function editMultiCourseMentorSPOC() {
    // Var name intentionally in global namespace as hook for tests to modify.
    window.gcb_multi_edit_dialog = new EditMultiCourseMentorSPOCPanel();
    window.gcb_multi_edit_dialog.init();
    window.gcb_multi_edit_dialog.open();
  };
  ...
  function bind() {
    ...
    $('#edit_multi_course_is_featured').click(
        editMultiCourseIsFeatured);
    $('#edit_multi_course_mentor_spoc').click(
        editMultiCourseMentorSPOC);
    ...
    $("[edit_course_spoc_mentor]")
        .append('<button ' +
            'class="gcb-list__icon gcb-list__icon--rowhover material-icons">' +
            'edit</button')
        .submit(function(event) {
          return confirm(
            'You are about to toggle the value for SPOC Update Mentor'
            +' for this course.  Really proceed?');
        });
