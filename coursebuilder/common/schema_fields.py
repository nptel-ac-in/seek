...
class SchemaField(Property):
    ...
    def _get_schema_dict(self, prefix_key, extra_select_options):
        ...
        elif self._select_data or extra_select_options:
            choices = []
            for value, label in self._select_data:
                choices.append(
                    {'value': value, 'label': unicode(label)})
            for value, label in extra_select_options:
                choices.append(
                    {'value': value, 'label': unicode(label)})
...
class FieldRegistry(Registry):
    ...
    def _get_schema_dict(self, prefix_key, extra_select_options):
        ...
        for schema_field in self._properties:
            sopts = dict()
            if schema_field.name in extra_select_options:
                sopts = extra_select_options[schema_field.name]
            key = base_key + [schema_field.name]
            # pylint: enable=protected-access
            schema_dict += schema_field._get_schema_dict(key, sopts)
        ...
        for key in self._sub_registries.keys():
            ...
            sopts = dict()
            if key in extra_select_options:
                sopts = extra_select_options[key]
    ...
    def get_schema_dict(self, extra_select_options=dict()):
        """Get schema dict for this API."""
        return self._get_schema_dict(list(), extra_select_options)
