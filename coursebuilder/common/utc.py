...
ALTERNATE_DATE_FMTS = ['%B %d, %Y', '%b %d, %Y']
...
def text_to_timestamp(text, fmt=ISO_8601_DATETIME_FMT):
    ...
    """
    utc_dt = None
    for fmt_ in [fmt] + ALTERNATE_DATE_FMTS:
        try:
            utc_dt = datetime.datetime.strptime(text, fmt_)
            break
        except ValueError as e:
            import logging
            logging.info("Failed to convert datetime: %s to %s, %s",
                         text, fmt_, str(e))
            continue
    if not utc_dt:
        raise ValueError("Failed to convert datetime: %s", text)
